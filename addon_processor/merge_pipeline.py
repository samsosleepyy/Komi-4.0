...
def _category_value(desc: Dict[str, Any]) -> str:
    menu_category = desc.get('menu_category') if isinstance(desc, dict) else None
    if isinstance(menu_category, dict):
        return str(menu_category.get('category') or '').strip().lower()
    if isinstance(menu_category, str):
        return menu_category.strip().lower()
    return ''


def _looks_like_ui_converted_addon(bp: Path) -> bool:
    """Detect packs generated by the UI conversion flow.

    Merge mode should keep only the already-UI-converted target items hidden.
    Normal uploaded addons are intentionally made visible in Creative after a merge.
    """
    try:
        manifest = _read_json(bp / 'manifest.json')
        header = manifest.get('header', {}) if isinstance(manifest, dict) else {}
        description = str(header.get('description') or '').lower() if isinstance(header, dict) else ''
        if 'normalize/rebuild ui addon generated by discord bot' in description:
            return True
    except Exception:
        pass
    ...
    if scripts_root.exists():
        for path in scripts_root.rglob('auto_ui_system.js'):
            try:
                text = path.read_text(encoding='utf-8', errors='ignore')
            except Exception:
                text = ''
            if 'menuItem' in text and 'armors' in text:
                return True
            # The file name alone is a strong signal for packs created by this bot.
            return True
    return False


def _collect_ui_hidden_item_ids(bp: Path, mapping: Dict[str, str]) -> Set[str]:
    """Return mapped item identifiers that must stay hidden after merging.

    This is only used for addons detected as outputs from the UI conversion flow.
    It preserves generated wearable/offhand target items with menu_category none,
    while selector items and normal addon items remain visible.
    """
    hidden: Set[str] = set()
    items_root = bp / 'items'
    ...
    for path in sorted(items_root.rglob('*.json')):
        ...
        if not identifier or identifier.startswith('minecraft:'):
            continue
        if _category_value(desc) == 'none':
            hidden.add(mapping.get(identifier, identifier))
    return hidden
...
def _normalize_merged_creative_visibility(merged_bp: Path, preserve_hidden_item_ids: Optional[Set[str]] = None) -> None:
    ...
    Normal source addons should keep their items visible in Equipment.
    If a source addon was already converted into the UI-selector format, only
    its generated target items that were hidden before the merge remain hidden.
    This keeps the selector visible while preventing duplicate UI target items
    from appearing in Creative.
    """
    preserve_hidden_item_ids = preserve_hidden_item_ids or set()
    ...
    for path in sorted(items_root.rglob('*.json')):
        ...
        if identifier.startswith('minecraft:'):
            continue
        if identifier in preserve_hidden_item_ids:
